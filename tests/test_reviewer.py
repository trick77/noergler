...
class TestStaleCommentResolution:
    @pytest.mark.asyncio
    async def test_stale_comments_resolved_on_rereview(self, mock_bitbucket, mock_copilot):
        """Old findings at different lines are resolved on re-review."""
        # New review returns finding at line 5
        mock_copilot.review_diff.return_value = _make_review_result([
            ReviewFinding(file="file.py", line=5, severity="warning", comment="New issue"),
        ])
        # Existing: old noergler comment at line 10 (stale)
        mock_bitbucket.fetch_pr_comments.return_value = [
            {
                "id": 100, "version": 1,
                "text": f"**Suggestion:** Old issue\n\n**Severity Level:** Warning ⚠️\n\n{NOERGLER_MARKER}",
                "path": "file.py", "line": 10, "parent_id": None,
            },
        ]
        mock_bitbucket.resolve_comment = AsyncMock(return_value=True)

        reviewer = Reviewer(mock_bitbucket, mock_copilot, _review_config())
        await reviewer.review_pull_request(_make_payload())

        mock_bitbucket.resolve_comment.assert_called_once_with("PROJ", "my-repo", 42, 100)

    @pytest.mark.asyncio
    async def test_current_findings_not_resolved(self, mock_bitbucket, mock_copilot):
        """Comments matching current findings are NOT resolved."""
        mock_copilot.review_diff.return_value = _make_review_result([
            ReviewFinding(file="file.py", line=10, severity="warning", comment="Same issue"),
        ])
        # Existing comment matches the current finding exactly
        mock_bitbucket.fetch_pr_comments.return_value = [
            {
                "id": 100, "version": 1,
                "text": f"**Suggestion:** Same issue\n\n**Severity Level:** Warning ⚠️\n\n{NOERGLER_MARKER}",
                "path": "file.py", "line": 10, "parent_id": None,
            },
        ]
        mock_bitbucket.resolve_comment = AsyncMock(return_value=True)
        ...
        mock_bitbucket.resolve_comment.assert_not_called()

    @pytest.mark.asyncio
    async def test_non_noergler_comments_not_resolved(self, mock_bitbucket, mock_copilot):
        """Human comments (without marker) are NOT resolved."""
        mock_copilot.review_diff.return_value = _make_review_result([
            ReviewFinding(file="file.py", line=5, severity="warning", comment="Issue"),
        ])
        mock_bitbucket.fetch_pr_comments.return_value = [
            {
                "id": 200, "version": 0,
                "text": "Human comment about line 10",
                "path": "file.py", "line": 10, "parent_id": None,
            },
        ]
        mock_bitbucket.resolve_comment = AsyncMock(return_value=True)
    ...
    @pytest.mark.asyncio
    async def test_summary_comments_not_resolved(self, mock_bitbucket, mock_copilot):
        """Summary comments (no path) are NOT resolved."""
        mock_copilot.review_diff.return_value = _make_review_result([
            ReviewFinding(file="file.py", line=5, severity="warning", comment="Issue"),
        ])
        mock_bitbucket.fetch_pr_comments.return_value = [
            {
                "id": 300, "version": 2,
                "text": f"🤖 **Review summary:** 1 issue found\n\n{NOERGLER_MARKER}",
