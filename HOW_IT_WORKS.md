...
**Skipped by extension:** Binary files (`.png`, `.jar`, `.exe`, etc.), lock files (`.lock`), minified files (`.min.js`), data files (`.json`, `.csv`), diagram sources (`.puml`, `.plantuml`, `.mmd`, `.drawio`, `.excalidraw`), generated output (`.snap`, `.po`, `.xlf`), and build config (`.properties`, `.bat`, `.cmd`). Lockfiles without a filterable extension (`go.sum`, `pnpm-lock.yaml`) are skipped by name. XML is *not* skipped — `pom.xml`, layout and config XML are reviewed like source.
