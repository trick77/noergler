...
// TestWrapProseGoldenCorpus is the parity oracle for the whole wrap path.
//
// testdata/wrap_golden.json was generated by running the real
// app/markdown_format.wrap_prose from the Python venv over 31 inputs; the
// generator is kept with the phase plan. Go has no textwrap, so the port is
// pinned byte for byte against the Python rather than against a reading of
// it. Five of the cases exist only to catch the Unicode-class divergences in
// the two structural regexes.
...
// A non-breaking space is not in textwrap._whitespace, so it never becomes a
// break point. strings.Fields would split on it.
...
// Python's \s is Unicode, so an NBSP-indented quote or heading is structural
// and passes through unwrapped. RE2's \s is not, hence [\s\p{Zs}].
...
// Python's \d is Unicode, so an Arabic-Indic marker is a numbered list item
// and wraps at the narrower list width. \p{Nd} restores that.
...
// str.expandtabs resets the column on \n and \r only; \v and \f are ordinary
// width-1 characters.
...
// The mask is padded to the span's real width so textwrap counts the span's
// true column width; an unpadded placeholder would let lines overflow.
