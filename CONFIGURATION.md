...
| `TEAM_<SLUG>_WEBHOOK_SECRET` | per team | Generated by the noergler admin: `openssl rand -hex 32`. The same value goes into the service's environment and, via `POST /onboard`, into every webhook of that team; the team admin never sees it |
...
Adding a team therefore means: generate the webhook secret, obtain the gateway key (and riptide token if used), put the three variables where the instance reads its environment, add the block to `teams.yaml`, redeploy; the team admin then calls `POST /onboard/<slug>` with their own Bitbucket token (never the bot token).
