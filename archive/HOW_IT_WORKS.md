...
**Skipped by extension:** Binary files (`.png`, `.jar`, `.exe`, etc.), lock files (`.lock`), minified files (`.min.js`), data files (`.json`, `.csv`), diagram sources (`.puml`, `.plantuml`, `.mmd`, `.drawio`, `.excalidraw`), generated output (`.snap`, `.ambr`, `.po`, `.xlf`), and build config (`.properties`, `.bat`, `.cmd`). Lockfiles without a filterable extension (`go.sum`, `pnpm-lock.yaml`) are skipped by name, as is protobuf compiler output (`*_pb2.py`, `*_pb2_grpc.py`) and `*.egg-info/` metadata. Alembic migrations under `alembic/versions/` are deliberately *not* skipped — autogenerated, but the likeliest place for irreversible data loss. XML is *not* skipped — `pom.xml`, layout and config XML are reviewed like source.
...
- When the endpoint reports nothing usable — a LiteLLM deployment its own cost map can't price sends the literal string `None` or no header at all; a proxy worker that started without its catalog does so for every call it serves — the run leaves `total_cost_usd` NULL and is never capped (fail-open). There is no rate table to estimate from. The client logs the raw pricing headers and the gateway's call id on every call (a warning when unpriced, naming the `x-litellm-*` headers that did arrive), the `Review complete` line carries the cost, and the startup ping warns once when the model is not priced. The `key total` is still shown on an unpriced run.
- A reported `0` on a call that consumed tokens is recorded as `$0.00` (the cap counts it) and logged as a warning: it usually means the gateway prices that deployment at zero.
